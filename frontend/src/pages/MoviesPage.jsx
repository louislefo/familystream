...
export default function MoviesPage() {
  ...
  const load = async (newPage = 1, append = false) => {
    ...
    try {
      let data
      if (genre !== 'all') {
        data = await getMoviesByGenre(genre, newPage)
      } else {
        const fn = sort === 'popular' ? getPopularMovies
          : sort === 'top' ? getTopRatedMovies
          : getNowPlayingMovies
        data = await fn(newPage)
      }
      const results = (data.results || []).map(i => ({ ...i, media_type: 'movie' }))
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto no-scrollbar pb-1">
          <button
            ...
            className={`shrink-0 px-4 py-2 rounded-xl text-sm font-medium cursor-pointer transition-all ${
              genre === 'all'
